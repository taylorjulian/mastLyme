...
# create master df
  ...
  tmp <- get(site) %>%  
    left_join(id, by = c('individualID', 'plotID')) %>%
    mutate(code = recode_factor(taxonID, `2PLANT` = 'unknUNKN', `ABIES` = 'abieUNKN', `ACER` = 'acerUNKN', `ACPE` = 'acerPens',
                                 `ACRU` = 'acerRubr', `ACSA2` = 'acerSac2', `ACSA3` = 'acerSacc', `ACSAS` = 'acerSacc', `BEAL2` = 'betuAlle',
                                 `BECAC` = 'betuUNKN', `BELE` = 'betuLent', `BEPA` = 'betuPapy', `BEPAC2` = 'betuPapy', `BEPAP` = 'betuPapy',
                                 `BEPO` = 'betuPopu', `FAGR` = 'faguGran', `FRAM2` = 'fraxAmer', `FRAXI` = 'fraxUNKN', `FRPE` = 'fraxPenn', 
  ...
  mutate(genus = recode_factor(taxonID, `2PLANT` = 'Unknown', `ABIES` = 'Abies', `ACER` = 'Acer', `ACPE` = 'Acer',
                                 ...
                                 `GYDI` = 'Gymnocladus', `LIVU` = 'Ligustrum', `MORUS` = 'Morus', `PODE3` = 'Populus', `ROAR3` = 'Rosa',
                                 `EUAT5` = 'Euonymus', `DIVI5` = 'Diospyros', `PIPU5` = 'Pinus')) %>%
    select(plotID, individualID, taxonID, code, genus, stemDiameter, measurementHeight, growthForm, plantStatus) %>%
    group_by(plotID, individualID, taxonID, code, genus, growthForm, plantStatus) %>%
...
acer <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(code = substr(parameter, 8, 15), parameter = substr(parameter, 17, nchar(parameter))) %>%
  mutate(parameter = as.factor(parameter)) %>%
...
asimina <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(code = substr(parameter, 8, 15), parameter = substr(parameter, 17, nchar(parameter))) %>%
  mutate(parameter = as.factor(parameter)) %>%
...
betula <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(code = substr(parameter, 8, 15), parameter = substr(parameter, 17, nchar(parameter))) %>%
  mutate(parameter = as.factor(parameter)) %>%
...
carya <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(code = substr(parameter, 8, 15), parameter = substr(parameter, 17, nchar(parameter))) %>%
  mutate(tmp = sapply(strsplit(parameter, split = 'lab:', fixed = TRUE), function(x) (x[2]))) %>%
...
cornus <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(code = substr(parameter, 8, 15), parameter = substr(parameter, 17, nchar(parameter))) %>%
  mutate(parameter = as.factor(parameter)) %>%
...
fagus <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(parameter = as.factor(parameter)) %>%
  mutate(code = 'faguGran') %>%
...
fraxinus <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(code = substr(parameter, 8, 15), parameter = substr(parameter, 17, nchar(parameter))) %>%
  mutate(parameter = as.factor(parameter)) %>%
...
ilex <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(code = substr(parameter, 8, 15), parameter = substr(parameter, 17, nchar(parameter))) %>%
  mutate(parameter = as.factor(parameter)) %>%
...
liriodendron <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(parameter = as.factor(parameter)) %>%
  mutate(code = 'liriTuli') %>%
...
liquidambar <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(parameter = as.factor(parameter)) %>%
  mutate(code = 'liquStyr') %>%
...
nyssa <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(parameter = as.factor(parameter)) %>%
  mutate(code = 'nyssSylv') %>%
...
oxydendrum <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(parameter = as.factor(parameter)) %>%
  mutate(code = 'oxydArbo') %>%
...
picea <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(code = substr(parameter, 8, 15), parameter = substr(parameter, 17, nchar(parameter))) %>%
  mutate(parameter = as.factor(parameter)) %>%
...
pinus <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(code = substr(parameter, 8, 15), parameter = substr(parameter, 17, nchar(parameter))) %>%
  mutate(parameter = as.factor(parameter)) %>%
...
quercus <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(code = substr(parameter, 8, 15), parameter = substr(parameter, 17, nchar(parameter))) %>%
  mutate(parameter = as.factor(parameter)) %>%
...
rhododenron <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(code = substr(parameter, 8, 15), parameter = substr(parameter, 17, nchar(parameter))) %>%
  mutate(parameter = as.factor(parameter)) %>%
...
tsuga <- output$parameters$betaRep %>%
  as.data.frame() %>%
  rownames_to_column(var = 'parameter') %>%
  mutate(parameter = as.factor(parameter)) %>%
  mutate(code = 'tsugCana') %>%
...
fecParams <- bind_rows(acer, betula, carya, cornus, fagus, fraxinus, ilex, liquidambar, 
                    liriodendron, nyssa, oxydendrum, picea, pinus, quercus, tsuga) %>%
  rename(interceptFec = '(Intercept)', defAnomaly_beta = defAnomaly, defSiteWinterTminSite_beta = 'defSite:winterTminSite', 
         defSite_beta = defSite, diam_betaFec = diam, diamSq_beta = 'I(diam^2)', 
         winterTminSite_beta = winterTminSite)

write.csv(fecParams, file = paste0(sppath, 'fecParams.csv'), row.names = FALSE)

repParams <- bind_rows(acer, betula, carya, cornus, fagus, fraxinus, ilex, liquidambar, 
                    liriodendron, nyssa, oxydendrum, picea, pinus, quercus, tsuga) %>%
...
load(paste0(srpath, 'pinusOutput.rdata'))  # BART, SCBI, GRSM, HARV, 
pine <- output

load(paste0(srpath, 'betulaOutput.rdata'))
betula <- output

inputs <- output$inputs$treeData %>%
  select(plot, year, winterTminSite, winterTminAnomaly, defSite, defAnomaly) %>%
  mutate(siteID = recode_factor(plot, `BARTC1P1` = 'BART', `HARVBW` = 'HARV', `SCBILFDP` = 'SCBI', `GRSWBIS` = 'GRSM')) 
```