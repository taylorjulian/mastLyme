...
cred <- pmTotalLag2Output$parameters$betaStandXWTable %>%
  ...
  mutate(species = recode_factor(species, `BLBR` = 'Blarina brevicauda', `PELE` = 'Peromyscus leucopus', `PEMA` = 'Peromyscus maniculatus',
                                 `SOCI` = 'Sorex cinereus', `TAST` = 'Tamias striatus')) %>%
...
cred <- mtHostOutput$parameters$betaStandXWTable %>%
  ...
  mutate(significance = as.factor(case_when(sig == -2 ~ 'negative relationship',
                                  ...
                                  sig == 2 ~ 'positive relationship'))) %>%
  select(-sig, -bottom, -top) %>%
  mutate(species = recode_factor(species, `BLBR` = 'Blarina brevicauda', `BLCA` = 'Blarina carolinensis', `BLHY` = 'Blarina hylophaga', 
                               `PELE` = 'Peromyscus leucopus', `PEMA` = 'Peromyscus maniculatus', `PEME` = 'Peromyscus merriami', 
                                `SOCI` = 'Sorex cinereus', `TAST` = 'Tamias striatus'))
...
mtSpecies <- ggplot(cred, aes(x = species, y = Estimate, col = significance)) + 
  geom_point() +
  facet_grid(vars(tick), labeller = labeller(tick = tickLab)) +
  geom_errorbar(aes(ymin = CI_025, ymax = CI_975), width = 0.1) +
...
ggsave(file = paste0(ipath, 'mtSpecies.png'), mtSpecies, device = NULL, width = 7.5, height = 7.5)
```

```{r phylogeny}
genera <- read.tree(text = '(((((((Fraxinus_-_ash, Ilex_-_holly), (Oxydendrum_-_sourwood, Rhododendron_-_rhododendron)), 
                  (Cornus_-_dogwood, Nyssa_-_black_gum)), ((((Fagus_-_beech, Quercus_-_oak), Carya_-_hickory), 
                  Betula_-_birch), Acer_-_maple)), Liquidambar_-_sweet_gum), ((Liriodendron_-_tulip_tree, Asimina_-_pawpaw), 
                  Lindera_-_spicebush)), ((Pinus_-_pine, Picea_-_spruce), Tsuga_-_hemlock));')
