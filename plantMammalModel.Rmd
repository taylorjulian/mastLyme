...
mammal <- bind_rows(BART, BLAN, GRSM, HARV, SCBI, SERC) %>%
  ...
  select(pair, year, total, totalTraps, BLBR, PELE, PEME, PEMA, SOCI, TAST) %>%
  group_by(year, pair) %>%
  summarise_all(sum) %>%
...
plantt_2 <- bind_rows(plant2016t_2, plant2015t_2, plant2014t_2, plant2013t_2, plant2012t_2) %>%
  select(pair, total)
...
ydata <- mammal %>%
  ungroup() %>%
  select(BLBR, PELE, PEME, PEMA, SOCI, TAST)
...
xdata <- bind_rows(plant2018, plant2017, plant2016, plant2015, plant2014, plant2013) %>%
  ungroup() %>%
  select(-year, -pair, -total)
xdata <- data.frame(xdata)

ydata <- mammal %>%
  ungroup %>%
  select(BLBR, PELE, TAST)
ydata <- data.frame(ydata)
...
n <- length(ydata$BLBR)
ef <- list(columns = 1:S, values = edata)
formula <- as.formula(~ Acer+Betula+Carya+Cornus+Fagus+Fraxinus+Ilex+Liquidambar+Nyssa+Picea+Pinus+Quercus+Tsuga)
modelList <- list(typeNames = c('DA'), effort = ef, ng = 10000, burnin = 1000)
